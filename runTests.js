...
async function runTests() {
    const results = [];
    const files = glob.sync('tests/sum-types/**/*.stella');
    const maxTests = 10; // Maximum number of tests to run
    ...
    for (const file of files) {
        ...
        try {
            ...
            if (isIllTyped) {
                results.push({ file, status: 'Failed', error: 'Expected failure for ill-typed test, but it passed.' });
            } else {
            ...
            if (isIllTyped) {
                ...
                results.push({ file, status: 'Failed', error: 'Expected success for well-typed test, but it failed.' });
            }
        }
